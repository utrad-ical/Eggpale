...
def codec(hps):
    ...
    # DCNN lock flags (when flag==1, DCNN is invalidated and not trained)
    levellockflags = [0] * hps.n_levels
    if 0 <= hps.highestleveltolock:
        for i in range(hps.n_levels):
            levellockflags[i] = tf.Variable(initial_value=0., trainable=False, name="levellockflag"+str(i))
            tf.add_to_collection("levellockers", 
                tf.assign(levellockflags[i], 1., name="levellocker"+str(i)))
            tf.add_to_collection("levelunlockers", 
                tf.assign(levellockflags[i], 0., name="levelunlocker"+str(i)))

    def encoder(z, objective):
        eps = []
        for i in range(hps.n_levels):
            z, objective = revnet2d(str(i), z, objective, hps, reverse=False, levellockflag=levellockflags[i])
            if i < hps.n_levels-1:
    ...
    def decoder(z, eps=[None]*hps.n_levels, eps_std=None):
        for i in reversed(range(hps.n_levels)):
            ...
            z, _ = revnet2d(str(i), z, 0, hps, reverse=True, levellockflag=levellockflags[i])
...
def model(sess, hps, train_iterator, test_iterator, data_init):
    ...
    with tf.name_scope('input'):
        X = tf.placeholder(
            tf.uint8, [None, hps.image_size, hps.image_size, hps.image_color_num], name='image')
        Y = tf.placeholder(tf.int32, [None], name='label')
        lr = tf.placeholder(tf.float32, None, name='learning_rate')
        X_float = tf.placeholder(
            tf.float32, [None, hps.image_size, hps.image_size, hps.image_color_num], name='image_float')
    ...
    if hps.inference or hps.inference_fast or hps.map or hps.inversemap or hps.interpolation:
        ...
        def f_encode(x, y, reuse=True):
            with tf.variable_scope('model', reuse=reuse):
                ...
                dLdx = t.gradient(objective, x)
                print("dLdx")
                print(dLdx)
                dLdx2 = tf.gradients(objective, x)[0]
                print("dLdx2")
                print(dLdx2)
            return eps, objective, dLdx2
        ...
        def f_calc_dLdx(x, y, reuse=True):
            with tf.variable_scope('model', reuse=reuse):
                y_onehot = tf.cast(tf.one_hot(y, hps.n_y, 1, 0), 'float32')
                ...
                if 0:
                    # z norm ver.
                    zsquarednorm = tf.zeros_like(objective)
                    for ep in eps:
                        print("ep")
                        print(ep)
                        sqep = tf.square(ep)
                        zsquarednorm += tf.reduce_sum(
                            tf.reduce_sum(
                                tf.reduce_sum(
                                    sqep, axis=3, keepdims=False
                                ), axis=2, keepdims=False
                            ), axis=1, keepdims=False
                        )
                    znorm = tf.sqrt(zsquarednorm)
                ...
                dLdx, = tf.gradients(objective, x, 
                    stop_gradients=[X_float, X, Y]+tf.trainable_variables())
                print("dLdx")
                print(dLdx)
            return dLdx, objective, eps
        
        enc_eps, objective_, _ = f_encode(X, Y)
        
        dLdx_, objective__, eps__ = f_calc_dLdx(X_float, Y)
        ...
        def objective(x, y):
            return sess.run(objective_, {X: x, Y: y})

        def dLdx(x_float, y):
            dLdx___, objective___, eps___ = sess.run((dLdx_, objective__, eps__), {X_float: x_float, Y: y})
            return (dLdx___, objective___, flatten_eps(eps___))

        def preprocess_cpu(x):
            x = x.astype(np.float32)
            if hps.n_bits_x < 8:
                x = np.floor(x / 2 ** (8 - hps.n_bits_x))
            x = x / hps.n_bins - .5
            return x
...
@add_arg_scope
def revnet2d(name, z, logdet, hps, reverse=False, levellockflag=0):
    with tf.variable_scope(name):
        if not reverse:
            for i in range(hps.depth):
                z, logdet = checkpoint(z, logdet)
                z, logdet = revnet2d_step(str(i), z, logdet, hps, reverse, levellockflag)
            z, logdet = checkpoint(z, logdet)
        else:
...
# Simpler, new version
@add_arg_scope
def revnet2d_step(name, z, logdet, hps, reverse, levellockflag=0):
    with tf.variable_scope(name):
        ...
        if not reverse:
            ...
            z = tf.check_numerics(z, "@@@ z inf/nan _ fore before actnorm @@@", "check1")
            z, logdet = Z.actnorm("actnorm", z, logdet=logdet)
            z = tf.check_numerics(z, "@@@ z inf/nan _ fore after actnorm @@@", "check2")
            ...
            z = tf.check_numerics(z, "@@@ z inf/nan _ fore after 1x1conv @@@", "check3")

            z1 = z[:, :, :, :n_z // 2]
            z2 = z[:, :, :, n_z // 2:]

            if hps.flow_coupling == 0:
                z2 += f("f1", z1, hps.width) * (1-levellockflag)
            elif hps.flow_coupling == 1:
                h = f("f1", z1, hps.width, n_z) * (1-levellockflag)
                shift = h[:, :, :, 0::2]
                # scale = tf.exp(h[:, :, :, 1::2])
                h = tf.check_numerics(h, "@@@ h inf/nan _ fore@@@", "check2")
                scale = tf.nn.sigmoid(h[:, :, :, 1::2] - .1) +.6
                scale = tf.check_numerics(scale, "@@@scale inf/nan _ fore@@@", "check4")
                z2 += shift
                z2 *= scale
            ...
            z1 = z[:, :, :, :n_z // 2]
            z1 = tf.check_numerics(z1, "@@@ z1 inf/nan _ back before 1x1conv @@@", "check4.5a")
            z2 = z[:, :, :, n_z // 2:]
            z2 = tf.check_numerics(z2, "@@@ z2 inf/nan _ back before 1x1conv @@@", "check4.5b")
            ...
            if hps.flow_coupling == 0:
                ...
                shift = h[:, :, :, 0::2]
                # scale = tf.exp(h[:, :, :, 1::2])
                h = tf.check_numerics(h, "@@@ h inf/nan _ back@@@", "check2")
                scale = tf.nn.sigmoid(h[:, :, :, 1::2] - .1) +.6
                scale = tf.check_numerics(scale, "@@@scale inf/nan _ back@@@", "check5")
                z2 /= scale
                z2 -= shift
                logdet -= tf.reduce_sum(tf.log(scale), axis=[1, 2, 3])
            else:
            ...
            z = tf.check_numerics(z, "@@@ z inf/nan _ back before 1x1conv @@@", "check6")
            ...
            if hps.flow_permutation == 0:
                ...
                z, logdet = invertible_1x1_conv(
                    "invconv", z, logdet, reverse=True, useSVD=True)
            elif hps.flow_permutation == 4:
                z, logdet = invertible_1x1_conv(
                    "invconv", z, logdet, reverse=True, usePI=True)                    
            else:
                raise Exception()

            z = tf.check_numerics(z, "@@@ z inf/nan _ back before actnorm @@@", "check7")
            z, logdet = Z.actnorm("actnorm", z, logdet=logdet, reverse=True)
            z = tf.check_numerics(z, "@@@ z inf/nan _ back after actnorm @@@", "check8")
...
def f(name, h, width, n_out=None):
    ...
    with tf.variable_scope(name):
        h = tf.check_numerics(h, "@@@ h inf/nan _ input@@@", "check1")
        h = tf.nn.relu(Z.conv2d("l_1", h, width))
        h = tf.check_numerics(h, "@@@ h inf/nan _ relu1@@@", "check2")
        h = tf.nn.relu(Z.conv2d("l_2", h, width, filter_size=[1, 1]))
        h = tf.check_numerics(h, "@@@ h inf/nan _ relu2@@@", "check3")
        h = Z.conv2d_zeros("l_last", h, n_out)
        h = tf.check_numerics(h, "@@@ h inf/nan _ zeros@@@", "check4")
    return h
...
# Invertible 1x1 conv
@add_arg_scope
def invertible_1x1_conv(name, z, logdet, reverse=False, useSVD=False, usePI=False):
    ...
    if True:  # Set to "False" to use the LU-decomposed version
        ...
        with tf.variable_scope(name):
            ...
            _w = w # this is deep copy !!

            if useSVD:
                minimalSingularValue = tf.constant(1.0e-3)
                maximalSingularValue = tf.constant(1.0e+3)

                # forcing w to be full rank

                # singular decomposition
                s, u, v = tf.linalg.svd(w, full_matrices=True, compute_uv=True)
                ssign = tf.sign(s)
                sabs = tf.abs(s)

                # forcing condition number > minimalConditionNumber
#                sabsmax = tf.reduce_max(sabs)
#                sabs = tf.maximum(sabs, sabsmax * minimalConditionNumber)
                sabs = tf.maximum(minimalSingularValue,
                                  tf.minimum(maximalSingularValue,
                                             sabs
                                             )
                                  )

                # forcing signs to be +1 or -1
                ssign = ssign + (1-ssign*ssign)

                # reconstruct w
                s_regularized = ssign * sabs
                w_regularized = tf.matmul(
                    u,
                    tf.matmul(
                        tf.linalg.diag(s_regularized),
                        v,
                        adjoint_b=True
                    )
                )

                # set regularized value to the weight matrix
                
                tf.add_to_collection(
                    "conv_weight_regularizer",
                    w.assign(w_regularized) # evaluation with delay
                )

            # @TODO check consistency between determinant and matrix inversion i.e., difference between w and _w
            det64 = tf.check_numerics(
            ...
            dlogdet = tf.check_numerics(
                tf.cast(tf.log(tf.abs(det64)), 'float32') * shape[1]*shape[2],
                "@@@LogDeterminant broken@@@",
                name="dlogdet"
            )

            ## @TODO ADD UNIT TESTS TO EVALUATE EXECUTION RESULT DIFFERENCES w.r.t. BELOW LINES

            # dlogdet = tf.linalg.LinearOperator(w).log_abs_determinant() * shape[1]*shape[2]
#            dlogdet = tf.cast(tf.log(abs(tf.matrix_determinant(
#                tf.cast(_w, 'float64')))), 'float32') * shape[1]*shape[2]
            ...
            if not reverse:
                ...
                if usePI: # @brief use pseudo-inverse instead 
                    wt = tf.transpose(_w)
                    _w = tf.matmul( tf.matrix_inverse(tf.matmul(wt, _w)), wt )
                else:
