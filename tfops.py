...
def actnorm_scale(name, x, scale=1., logdet=None, logscale_factor=3., batch_variance=False, reverse=False, init=False, trainable=True):
    ...
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE), arg_scope([get_variable_ddi], trainable=trainable):
        ...
        if True:
            ...
            logs = tf.check_numerics(logs, "@@@ logs inf/nan@@@", "check_a")
            ...
            # actnorm scale factor regularizer
            logs_old = tf.get_variable("logs", _shape, tf.float32, None, 
                trainable=trainable) # get without initializing
            logs_absmax = np.log(1e+2).astype("float32")
            logs_new = tf.minimum(logs_absmax, 
                        tf.maximum(-logs_absmax, logs_old)
            )
            
            logs_new = tf.check_numerics(logs_new, "@@@ logs_new inf/nan@@@", "check_b")

            tf.add_to_collection("actnorm_regularizer", 
                logs_old.assign(logs_new))
...
def conv2d_zeros(name, x, width, filter_size=[3, 3], stride=[1, 1], pad="SAME", logscale_factor=3, skip=1, edge_bias=True):
    with tf.variable_scope(name):
        ...
        x += tf.get_variable("b", [1, 1, 1, width],
                             initializer=tf.zeros_initializer())
        x *= tf.exp(tf.get_variable("logs",
                                    [1, 1, 1, width], initializer=tf.zeros_initializer()) * logscale_factor)
    return x
